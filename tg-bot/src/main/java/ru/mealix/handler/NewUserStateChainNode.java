...
public class NewUserStateChainNode extends ChainNode {
    private final ConcurrentHashMap<Language, ConcurrentHashMap<String, Command>> commands;


    public NewUserStateChainNode(ChainNode next, UserService userService, ConcurrentHashMap<Language, ConcurrentHashMap<String, Command>> commands) {
        super(next, userService);
        this.commands = commands;
    }

    public NewUserStateChainNode(UserService userService, ConcurrentHashMap<Language, ConcurrentHashMap<String, Command>> commands) {
        super(userService);
        this.commands = commands;
    }
    ...
    protected boolean process(Update update, DefaultAbsSender client) throws TelegramApiException {
        ...
        if (text.equals("/start")) {
            ...
            command.get("/start").execute(update, client);
            return true;
        }
        return false;
    }
